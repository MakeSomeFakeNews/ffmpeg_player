...
void
LowLagDecoder::setOutputSurface(JNIEnv *env, jobject surface) {
    USE_SW_DECODER_INSTEAD = true;
    if (surface == nullptr) {
...
void
LowLagDecoder::registerOnDecoderRatioChangedCallback(DECODER_RATIO_CHANGED decoderRatioChangedC) {
    onDecoderRatioChangedCallback = std::move(decoderRatioChangedC);
}

void LowLagDecoder::registerOnDecodingInfoChangedCallback(
